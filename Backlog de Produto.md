...
5. **Links Externos para Desenvolver Atividades:**
    ...
    - **Exemplo:**
        - [Documentação da biblioteca XYZ](https://www.notion.so/URL_DOCUMENTACAO)
        - [Tutorial de integração com OAuth](https://www.notion.so/URL_TUTORIAL)
6. **Dependências:**
...
1. **Link da Atividade no GitHub:**
    - [Link para a atividade no GitHub](https://github.com/exemplo/projeto/atividade)
2. **Link do Figma (Caso Necessário):**
    - [Link para o design no Figma](https://www.figma.com/file/exemplo)
3. **Descrição da Atividade a Ser Desenvolvida:**
    - "Implementar a funcionalidade de login com OAuth 2.0. Isso inclui configurar a biblioteca XYZ, criar endpoints para autenticação e integrar com o serviço de identidade ABC."
4. **Critérios de Aceitação:**
    - "Usuário deve conseguir fazer login com OAuth sem erros."
    - "Os dados do usuário devem ser salvos corretamente no banco de dados."
    - "Deve haver feedback visual para erros de login."
5. **Links Externos para Desenvolver Atividades:**
    - [Documentação da biblioteca XYZ](https://documentacao.xyz/)
    - [Tutorial de integração com OAuth](https://tutorial.oauth/)
6. **Dependências:**
    - "Esta tarefa depende da conclusão da implementação do backend para autenticação."
7. **Sub Tarefas:**
    - "Configurar ambiente de desenvolvimento"
    - "Implementar interface de login"
    - "Testar integração OAuth"
8. **Comentários Adicionais:**
    - "Verificar se a autenticação está compatível com o sistema atual."